...
const replaceT = (currentFile: string, cssMap: Map<string, string>, replaceVar: string) => {
  let template = currentFile
  cssMap.forEach((v, k) => {
    const lastK = k.split(' ').slice(-1)[0]
    const className = `${replaceVar}${lastK}`
    template = currentFile.replace(className, `'${v}'`)
  })
  return template
}
...
describe('aa', () => {
  it('should', async () => {
    const map = await main(cssFile)
    const data = replaceT(tempalte, map, 'style')
